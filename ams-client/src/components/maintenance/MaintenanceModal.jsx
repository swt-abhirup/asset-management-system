import { useEffect, useState } from "react";
import { X, Wrench } from "lucide-react";
import api from "../../services/api";
import { toast } from "../Toast";

const L = { color: "#1b4f72", fontSize: "11px", fontWeight: 600, marginBottom: "4px", display: "block" };
const I = { width: "100%", padding: "6px 10px", fontSize: "12px", border: "1px solid #e2e8f0", borderRadius: "6px", outline: "none", color: "#1e293b", backgroundColor: "#fff" };
const fi = e => { e.target.style.borderColor = "#19405e"; e.target.style.boxShadow = "0 0 0 2px rgba(25,64,94,0.1)"; };
const fo = e => { e.target.style.borderColor = "#e2e8f0"; e.target.style.boxShadow = "none"; };

const STATUS_COLORS = {
    scheduled:   { bg: "rgba(245,203,167,0.3)",  color: "#7a4a1e" },
    "in-progress":{ bg: "rgba(230,126,34,0.15)", color: "#9a4a10" },
    completed:   { bg: "rgba(25,64,94,0.1)",     color: "#19405e" },
};

export default function MaintenanceModal({ log, assetName, onClose, onSaved }) {

    const [form,   setForm]   = useState({ status: "", cost: "", completed_date: "", notes: "" });
    const [saving, setSaving] = useState(false);

    useEffect(() => {
        if (log) setForm({
            status:         log.status         ?? "scheduled",
            cost:           log.cost           ?? "",
            completed_date: log.completed_date ?? "",
            notes:          log.notes          ?? "",
        });
    }, [log]);

    const set = e => setForm(f => ({ ...f, [e.target.name]: e.target.value }));

    const update = async () => {
        try {
            setSaving(true);
            await api.put(`/api/maintenance/${log.log_id}`, {
                status:         form.status,
                cost:           form.cost ? Number(form.cost) : log.cost,
                completed_date: form.completed_date || null,
                notes:          form.notes,
            });
            onClose();
            onSaved();
            setTimeout(() => toast.success(
                form.status === "completed"
                    ? "Maintenance completed — asset marked available."
                    : "Maintenance log updated."
            ), 0);
        } catch (err) {
            toast.error(err.response?.data?.message || "Update failed.");
        } finally {
            setSaving(false);
        }
    };

    if (!log) return null;

    const sc = STATUS_COLORS[log.status] ?? STATUS_COLORS.scheduled;

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center"
            style={{ backgroundColor: "rgba(0,0,0,0.45)" }}>

            <div className="w-full max-w-lg rounded-xl shadow-2xl" style={{ backgroundColor: "#ffffff" }}>

                {/* Header */}
                <div className="flex items-center justify-between px-5 py-3.5 border-b"
                    style={{ borderColor: "#f1f5f9" }}>
                    <div className="flex items-center gap-2">
                        <Wrench size={14} style={{ color: "#19405e" }} />
                        <h3 className="text-sm font-bold"
                            style={{ fontFamily: "'Bricolage Grotesque', sans-serif", color: "#19405e" }}>
                            Update Maintenance Log
                        </h3>
                    </div>
                    <button onClick={onClose} className="w-6 h-6 rounded flex items-center justify-center"
                        style={{ color: "#94a3b8" }}
                        onMouseEnter={e => { e.currentTarget.style.backgroundColor = "#f1f5f9"; e.currentTarget.style.color = "#19405e"; }}
                        onMouseLeave={e => { e.currentTarget.style.backgroundColor = "transparent"; e.currentTarget.style.color = "#94a3b8"; }}>
                        <X size={14} />
                    </button>
                </div>

                {/* Body */}
                <div className="px-5 py-4 flex flex-col gap-3">

                    {/* Summary strip */}
                    <div className="rounded-lg p-3 flex items-start justify-between"
                        style={{ backgroundColor: "#f8fafc", border: "1px solid #e2e8f0" }}>
                        <div>
                            <p className="text-xs font-semibold" style={{ color: "#19405e" }}>{assetName}</p>
                            <p className="text-xs mt-0.5" style={{ color: "#64748b" }}>
                                <span className="capitalize">{log.type}</span>
                                {log.vendor ? ` · ${log.vendor}` : ""}
                            </p>
                            <p className="text-xs mt-1" style={{ color: "#94a3b8" }}>{log.description}</p>
                        </div>
                        <span className="px-2 py-0.5 rounded text-xs font-semibold capitalize ml-3 flex-shrink-0"
                            style={{ backgroundColor: sc.bg, color: sc.color }}>
                            {log.status}
                        </span>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label style={L}>Update Status</label>
                            <select name="status" value={form.status} onChange={set} style={I}>
                                <option value="scheduled">Scheduled</option>
                                <option value="in-progress">In Progress</option>
                                <option value="completed">Completed</option>
                            </select>
                        </div>
                        <div>
                            <label style={L}>Actual Cost (₹)</label>
                            <input type="number" name="cost" value={form.cost} onChange={set}
                                placeholder={log.cost || "0"} style={I} onFocus={fi} onBlur={fo} />
                        </div>
                    </div>

                    <div>
                        <label style={L}>Completed Date</label>
                        <input type="date" name="completed_date" value={form.completed_date} onChange={set}
                            style={I} onFocus={fi} onBlur={fo} />
                    </div>

                    <div>
                        <label style={L}>Resolution Notes</label>
                        <textarea name="notes" rows={3} value={form.notes} onChange={set}
                            placeholder="What was done, parts replaced, outcome…"
                            style={{ ...I, resize: "none" }} onFocus={fi} onBlur={fo} />
                    </div>

                    {form.status === "completed" && (
                        <p className="text-xs px-3 py-2 rounded"
                            style={{ backgroundColor: "rgba(25,64,94,0.07)", color: "#19405e", border: "1px solid rgba(25,64,94,0.15)" }}>
                            ✓ Marking as completed will set the asset back to <strong>available</strong>.
                        </p>
                    )}
                </div>

                {/* Footer */}
                <div className="flex justify-end gap-2 px-5 py-3 border-t" style={{ borderColor: "#f1f5f9" }}>
                    <button onClick={onClose}
                        className="px-4 py-2 rounded text-xs font-semibold border"
                        style={{ borderColor: "#e2e8f0", color: "#64748b" }}
                        onMouseEnter={e => e.currentTarget.style.backgroundColor = "#f8fafc"}
                        onMouseLeave={e => e.currentTarget.style.backgroundColor = "transparent"}>
                        Cancel
                    </button>
                    <button onClick={update} disabled={saving}
                        className="px-4 py-2 rounded text-xs font-semibold text-white disabled:opacity-60"
                        style={{ backgroundColor: "#19405e" }}
                        onMouseEnter={e => { if (!saving) e.currentTarget.style.backgroundColor = "#1b4f72"; }}
                        onMouseLeave={e => { if (!saving) e.currentTarget.style.backgroundColor = "#19405e"; }}>
                        {saving ? "Saving…" : "Update Log"}
                    </button>
                </div>
            </div>
        </div>
    );
}

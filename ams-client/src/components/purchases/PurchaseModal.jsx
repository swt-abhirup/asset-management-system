...
import { X, ShoppingCart, Plus, Trash2, RefreshCw } from "lucide-react";
...
const IS_RO = { ...IS, backgroundColor: "#f0f7ff", color: "#19405e", fontWeight: 600, cursor: "default" };
...
export default function PurchaseModal({ purchase, vendors, onClose, onSaved }) {
    ...
    // Fetch auto-generated numbers for new orders
    const fetchNextNumbers = async () => {
        try {
            setLoadingNums(true);
            const res = await api.get("/api/purchases/next-numbers");
            setForm(f => ({
                ...f,
            ...
            toast.warn("Could not auto-generate PO/INV numbers.");
        } finally {
    ...
    useEffect(() => {
        if (purchase) {
            ...
            fetchNextNumbers();   // auto-fill on open for new orders
        }
    }, [purchase]);
                ...
                <div className="overflow-y-auto flex-1 px-5 py-4 flex flex-col gap-4">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                            ...
                            {/* PO Number — auto-generated, read-only for new, editable for edit */}
                            <div>
                                <div className="flex items-center justify-between mb-1">
                                    <label style={{ ...L, marginBottom: 0 }}>PO Number</label>
                                    {!isEdit && (
                                        <button onClick={fetchNextNumbers} disabled={loadingNums}
                                            className="flex items-center gap-1 text-xs"
                                            style={{ color: "#1b4f72" }}
                                            title="Regenerate">
                                            <RefreshCw size={10} className={loadingNums ? "animate-spin" : ""} />
                                        </button>
                                    )}
                                </div>
                                <input name="po_number" value={form.po_number}
                                    onChange={isEdit ? set : undefined}
                                    readOnly={!isEdit}
                                    style={!isEdit ? IS_RO : IS}
                                    onFocus={isEdit ? fi : undefined}
                                    onBlur={isEdit ? fo : undefined}
                                    placeholder={loadingNums ? "Generating…" : ""}
                                />
                            </div>

                            {/* Invoice Number — auto-generated, read-only for new */}
                            <div>
                            ...
                            {form.items.map((item, idx) => (
                                <div key={idx} className="grid gap-2 mb-2 items-center min-w-[480px]"
                                    ...
                                    <input type="number" value={item.unit_price}
                                        ...
                                        placeholder="0" style={IS} onFocus={fi} onBlur={fo} />
